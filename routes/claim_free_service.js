...
router.post("/", (req, res) => {
    ...
    mongo.connect(process.env.CONNECTION_STRING, (err, dbo) => {
      ...
      let { name, email, company, phone, projectName, projectDesc } = req.body;
        ...
        .then((data) => {
          ...
          async function main() {
            ...
            let info = await transporter.sendMail({
              ...
              html: `<p>Sender_Name: ${name} </p> <p>Sender_Email: ${email} </p>  <p> company: ${company} </p> <p> phone: ${phone} </p> <p> projectName: ${projectName} </p> <p> projectDesc: ${projectDesc} </p>` // html body
            });
